...
- [ ] **Step 2: Prep the turtle** — diamond pickaxe + wireless modem equipped, inventory 1: stone, 2: empty bucket, 4: coal. Place at position A facing the conversion spot. `label set AmalgamT1`, then the standard bootstrap (`wget .../update.lua update`, `update`), `edit startup` → `shell.run("update")` / `shell.run("amalgamturtle")`.
